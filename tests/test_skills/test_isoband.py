"""Smoke tests for the isoband skill.

Skipped when Rscript is missing or the upstream `isoband` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("isoband", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream isoband package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("isoband", {"fn": "does_not_exist_certainly_xyz"})
