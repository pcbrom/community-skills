"""Smoke tests for the readr skill.

Skipped when Rscript is missing or the upstream `readr` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("readr", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream readr package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("readr", {"fn": "does_not_exist_certainly_xyz"})
