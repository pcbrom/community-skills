"""Smoke tests for the readxl skill.

Skipped when Rscript is missing or the upstream `readxl` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("readxl", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream readxl package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("readxl", {"fn": "does_not_exist_certainly_xyz"})
