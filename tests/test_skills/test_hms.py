"""Smoke tests for the hms skill.

Skipped when Rscript is missing or the upstream `hms` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("hms", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream hms package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("hms", {"fn": "does_not_exist_certainly_xyz"})
