"""Smoke tests for the rprojroot skill.

Skipped when Rscript is missing or the upstream `rprojroot` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("rprojroot", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream rprojroot package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("rprojroot", {"fn": "does_not_exist_certainly_xyz"})
