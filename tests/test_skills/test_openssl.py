"""Smoke tests for the openssl skill.

Skipped when Rscript is missing or the upstream `openssl` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("openssl", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream openssl package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("openssl", {"fn": "does_not_exist_certainly_xyz"})
