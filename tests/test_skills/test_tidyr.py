"""Smoke tests for the tidyr skill.

Skipped when Rscript is missing or the upstream `tidyr` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("tidyr", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream tidyr package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("tidyr", {"fn": "does_not_exist_certainly_xyz"})
