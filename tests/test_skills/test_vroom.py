"""Smoke tests for the vroom skill.

Skipped when Rscript is missing or the upstream `vroom` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("vroom", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream vroom package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("vroom", {"fn": "does_not_exist_certainly_xyz"})
