"""Smoke tests for the withr skill.

Skipped when Rscript is missing or the upstream `withr` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("withr", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream withr package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("withr", {"fn": "does_not_exist_certainly_xyz"})
