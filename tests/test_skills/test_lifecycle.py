"""Smoke tests for the lifecycle skill.

Skipped when Rscript is missing or the upstream `lifecycle` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("lifecycle", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream lifecycle package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("lifecycle", {"fn": "does_not_exist_certainly_xyz"})
