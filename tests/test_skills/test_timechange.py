"""Smoke tests for the timechange skill.

Skipped when Rscript is missing or the upstream `timechange` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("timechange", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream timechange package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("timechange", {"fn": "does_not_exist_certainly_xyz"})
