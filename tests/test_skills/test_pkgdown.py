"""Smoke tests for the pkgdown skill.

Skipped when Rscript is missing or the upstream `pkgdown` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("pkgdown", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream pkgdown package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("pkgdown", {"fn": "does_not_exist_certainly_xyz"})
