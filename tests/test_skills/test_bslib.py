"""Smoke tests for the bslib skill.

Skipped when Rscript is missing or the upstream `bslib` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("bslib", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream bslib package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("bslib", {"fn": "does_not_exist_certainly_xyz"})
