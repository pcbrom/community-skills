"""Smoke tests for the pkgload skill.

Skipped when Rscript is missing or the upstream `pkgload` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("pkgload", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream pkgload package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("pkgload", {"fn": "does_not_exist_certainly_xyz"})
