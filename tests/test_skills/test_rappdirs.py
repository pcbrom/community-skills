"""Smoke tests for the rappdirs skill.

Skipped when Rscript is missing or the upstream `rappdirs` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("rappdirs", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream rappdirs package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("rappdirs", {"fn": "does_not_exist_certainly_xyz"})
