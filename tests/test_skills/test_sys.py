"""Smoke tests for the sys skill.

Skipped when Rscript is missing or the upstream `sys` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("sys", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream sys package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("sys", {"fn": "does_not_exist_certainly_xyz"})
