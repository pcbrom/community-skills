"""Smoke tests for the crayon skill.

Skipped when Rscript is missing or the upstream `crayon` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("crayon", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream crayon package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("crayon", {"fn": "does_not_exist_certainly_xyz"})
