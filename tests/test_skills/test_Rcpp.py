"""Smoke tests for the Rcpp skill.

Skipped when Rscript is missing or the upstream `Rcpp` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("Rcpp", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream Rcpp package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("Rcpp", {"fn": "does_not_exist_certainly_xyz"})
