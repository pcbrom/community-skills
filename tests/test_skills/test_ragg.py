"""Smoke tests for the ragg skill.

Skipped when Rscript is missing or the upstream `ragg` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("ragg", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream ragg package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("ragg", {"fn": "does_not_exist_certainly_xyz"})
