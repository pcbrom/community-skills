"""Smoke tests for the data.table skill.

Skipped when Rscript is missing or the upstream `data.table` package is
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream data.table package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("data.table", {"fn": "does_not_exist_certainly_xyz"})
