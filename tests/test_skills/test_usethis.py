"""Smoke tests for the usethis skill.

Skipped when Rscript is missing or the upstream `usethis` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("usethis", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream usethis package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("usethis", {"fn": "does_not_exist_certainly_xyz"})
