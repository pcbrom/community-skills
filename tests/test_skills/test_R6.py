"""Smoke tests for the R6 skill.

Skipped when Rscript is missing or the upstream `R6` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("R6", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream R6 package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("R6", {"fn": "does_not_exist_certainly_xyz"})
