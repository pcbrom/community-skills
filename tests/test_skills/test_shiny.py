"""Smoke tests for the shiny skill.

Skipped when Rscript is missing or the upstream `shiny` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("shiny", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream shiny package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("shiny", {"fn": "does_not_exist_certainly_xyz"})
