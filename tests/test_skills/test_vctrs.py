"""Smoke tests for the vctrs skill.

Skipped when Rscript is missing or the upstream `vctrs` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("vctrs", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream vctrs package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("vctrs", {"fn": "does_not_exist_certainly_xyz"})
