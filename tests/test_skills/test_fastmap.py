"""Smoke tests for the fastmap skill.

Skipped when Rscript is missing or the upstream `fastmap` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("fastmap", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream fastmap package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("fastmap", {"fn": "does_not_exist_certainly_xyz"})
