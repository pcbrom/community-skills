"""Smoke tests for the RcppEigen skill.

Skipped when Rscript is missing or the upstream `RcppEigen` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("RcppEigen", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream RcppEigen package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("RcppEigen", {"fn": "does_not_exist_certainly_xyz"})
