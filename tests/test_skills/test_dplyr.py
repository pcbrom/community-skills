"""Smoke tests for the dplyr skill.

Skipped when Rscript is missing or the upstream `dplyr` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("dplyr", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream dplyr package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("dplyr", {"fn": "does_not_exist_certainly_xyz"})
