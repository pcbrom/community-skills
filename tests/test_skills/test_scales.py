"""Smoke tests for the scales skill.

Skipped when Rscript is missing or the upstream `scales` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("scales", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream scales package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("scales", {"fn": "does_not_exist_certainly_xyz"})
