"""Smoke tests for the tibble skill.

Skipped when Rscript is missing or the upstream `tibble` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("tibble", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream tibble package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("tibble", {"fn": "does_not_exist_certainly_xyz"})
