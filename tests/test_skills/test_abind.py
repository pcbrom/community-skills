"""Smoke tests for the abind skill.

Skipped when Rscript is missing or the upstream `abind` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("abind", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream abind package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("abind", {"fn": "does_not_exist_certainly_xyz"})
