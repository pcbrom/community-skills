"""Smoke tests for the commonmark skill.

Skipped when Rscript is missing or the upstream `commonmark` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("commonmark", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream commonmark package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("commonmark", {"fn": "does_not_exist_certainly_xyz"})
