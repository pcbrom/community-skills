"""Smoke tests for the pillar skill.

Skipped when Rscript is missing or the upstream `pillar` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("pillar", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream pillar package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("pillar", {"fn": "does_not_exist_certainly_xyz"})
