"""Smoke tests for the later skill.

Skipped when Rscript is missing or the upstream `later` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("later", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream later package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("later", {"fn": "does_not_exist_certainly_xyz"})
