"""Smoke tests for the devtools skill.

Skipped when Rscript is missing or the upstream `devtools` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("devtools", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream devtools package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("devtools", {"fn": "does_not_exist_certainly_xyz"})
