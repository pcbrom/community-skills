"""Smoke tests for the tzdb skill.

Skipped when Rscript is missing or the upstream `tzdb` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("tzdb", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream tzdb package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("tzdb", {"fn": "does_not_exist_certainly_xyz"})
