"""Smoke tests for the pkgbuild skill.

Skipped when Rscript is missing or the upstream `pkgbuild` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("pkgbuild", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream pkgbuild package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("pkgbuild", {"fn": "does_not_exist_certainly_xyz"})
