"""Smoke tests for the S7 skill.

Skipped when Rscript is missing or the upstream `S7` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("S7", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream S7 package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("S7", {"fn": "does_not_exist_certainly_xyz"})
