"""Smoke tests for the bit skill.

Skipped when Rscript is missing or the upstream `bit` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("bit", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream bit package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("bit", {"fn": "does_not_exist_certainly_xyz"})
