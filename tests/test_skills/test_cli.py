"""Smoke tests for the cli skill.

Skipped when Rscript is missing or the upstream `cli` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("cli", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream cli package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("cli", {"fn": "does_not_exist_certainly_xyz"})
