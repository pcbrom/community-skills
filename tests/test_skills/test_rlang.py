"""Smoke tests for the rlang skill.

Skipped when Rscript is missing or the upstream `rlang` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("rlang", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream rlang package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("rlang", {"fn": "does_not_exist_certainly_xyz"})
