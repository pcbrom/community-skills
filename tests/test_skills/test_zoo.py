"""Smoke tests for the zoo skill.

Skipped when Rscript is missing or the upstream `zoo` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("zoo", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream zoo package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("zoo", {"fn": "does_not_exist_certainly_xyz"})
