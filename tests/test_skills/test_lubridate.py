"""Smoke tests for the lubridate skill.

Skipped when Rscript is missing or the upstream `lubridate` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("lubridate", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream lubridate package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("lubridate", {"fn": "does_not_exist_certainly_xyz"})
