"""Smoke tests for the brms skill.

Skipped when Rscript is missing or the upstream `brms` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("brms", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream brms package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("brms", {"fn": "does_not_exist_certainly_xyz"})
