"""Smoke tests for the progress skill.

Skipped when Rscript is missing or the upstream `progress` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("progress", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream progress package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("progress", {"fn": "does_not_exist_certainly_xyz"})
