"""Smoke tests for the fs skill.

Skipped when Rscript is missing or the upstream `fs` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("fs", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream fs package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("fs", {"fn": "does_not_exist_certainly_xyz"})
