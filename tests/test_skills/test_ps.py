"""Smoke tests for the ps skill.

Skipped when Rscript is missing or the upstream `ps` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("ps", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream ps package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("ps", {"fn": "does_not_exist_certainly_xyz"})
