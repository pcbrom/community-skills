"""Smoke tests for the broom skill.

Skipped when Rscript is missing or the upstream `broom` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("broom", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream broom package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("broom", {"fn": "does_not_exist_certainly_xyz"})
