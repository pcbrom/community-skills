"""Smoke tests for the yaml skill.

Skipped when Rscript is missing or the upstream `yaml` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("yaml", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream yaml package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("yaml", {"fn": "does_not_exist_certainly_xyz"})
