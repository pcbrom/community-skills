"""Smoke tests for the digest skill.

Skipped when Rscript is missing or the upstream `digest` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("digest", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream digest package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("digest", {"fn": "does_not_exist_certainly_xyz"})
