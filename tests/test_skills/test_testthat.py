"""Smoke tests for the testthat skill.

Skipped when Rscript is missing or the upstream `testthat` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("testthat", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream testthat package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("testthat", {"fn": "does_not_exist_certainly_xyz"})
