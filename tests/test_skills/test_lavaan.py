"""Smoke tests for the lavaan skill.

Skipped when Rscript is missing or the upstream `lavaan` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("lavaan", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream lavaan package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("lavaan", {"fn": "does_not_exist_certainly_xyz"})
