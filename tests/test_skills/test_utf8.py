"""Smoke tests for the utf8 skill.

Skipped when Rscript is missing or the upstream `utf8` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("utf8", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream utf8 package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("utf8", {"fn": "does_not_exist_certainly_xyz"})
