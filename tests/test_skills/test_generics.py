"""Smoke tests for the generics skill.

Skipped when Rscript is missing or the upstream `generics` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("generics", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream generics package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("generics", {"fn": "does_not_exist_certainly_xyz"})
