"""Smoke tests for the knitr skill.

Skipped when Rscript is missing or the upstream `knitr` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("knitr", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream knitr package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("knitr", {"fn": "does_not_exist_certainly_xyz"})
