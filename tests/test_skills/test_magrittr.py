"""Smoke tests for the magrittr skill.

Skipped when Rscript is missing or the upstream `magrittr` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("magrittr", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream magrittr package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("magrittr", {"fn": "does_not_exist_certainly_xyz"})
