"""Smoke tests for the lintr skill.

Skipped when Rscript is missing or the upstream `lintr` package is
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream lintr package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("lintr", {"fn": "does_not_exist_certainly_xyz"})
