"""Smoke tests for the evaluate skill.

Skipped when Rscript is missing or the upstream `evaluate` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("evaluate", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream evaluate package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("evaluate", {"fn": "does_not_exist_certainly_xyz"})
