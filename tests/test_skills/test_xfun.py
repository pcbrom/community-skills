"""Smoke tests for the xfun skill.

Skipped when Rscript is missing or the upstream `xfun` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("xfun", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream xfun package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("xfun", {"fn": "does_not_exist_certainly_xyz"})
