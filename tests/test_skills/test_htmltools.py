"""Smoke tests for the htmltools skill.

Skipped when Rscript is missing or the upstream `htmltools` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("htmltools", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream htmltools package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("htmltools", {"fn": "does_not_exist_certainly_xyz"})
