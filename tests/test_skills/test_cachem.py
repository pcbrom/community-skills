"""Smoke tests for the cachem skill.

Skipped when Rscript is missing or the upstream `cachem` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("cachem", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream cachem package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("cachem", {"fn": "does_not_exist_certainly_xyz"})
