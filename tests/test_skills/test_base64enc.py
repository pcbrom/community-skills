"""Smoke tests for the base64enc skill.

Skipped when Rscript is missing or the upstream `base64enc` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("base64enc", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream base64enc package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("base64enc", {"fn": "does_not_exist_certainly_xyz"})
