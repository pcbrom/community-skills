"""Smoke tests for the callr skill.

Skipped when Rscript is missing or the upstream `callr` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("callr", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream callr package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("callr", {"fn": "does_not_exist_certainly_xyz"})
