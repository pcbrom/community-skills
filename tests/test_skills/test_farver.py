"""Smoke tests for the farver skill.

Skipped when Rscript is missing or the upstream `farver` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("farver", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream farver package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("farver", {"fn": "does_not_exist_certainly_xyz"})
