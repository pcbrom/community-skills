"""Smoke tests for the rmarkdown skill.

Skipped when Rscript is missing or the upstream `rmarkdown` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("rmarkdown", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream rmarkdown package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("rmarkdown", {"fn": "does_not_exist_certainly_xyz"})
