"""Smoke tests for the RefManageR skill.

Skipped when Rscript is missing or the upstream `RefManageR` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("RefManageR", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream RefManageR package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("RefManageR", {"fn": "does_not_exist_certainly_xyz"})
