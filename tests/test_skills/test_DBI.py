"""Smoke tests for the DBI skill.

Skipped when Rscript is missing or the upstream `DBI` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("DBI", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream DBI package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("DBI", {"fn": "does_not_exist_certainly_xyz"})
