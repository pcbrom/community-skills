"""Smoke tests for the purrr skill.

Skipped when Rscript is missing or the upstream `purrr` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("purrr", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream purrr package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("purrr", {"fn": "does_not_exist_certainly_xyz"})
