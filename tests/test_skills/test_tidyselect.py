"""Smoke tests for the tidyselect skill.

Skipped when Rscript is missing or the upstream `tidyselect` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("tidyselect", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream tidyselect package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("tidyselect", {"fn": "does_not_exist_certainly_xyz"})
