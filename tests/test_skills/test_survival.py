"""Smoke tests for the survival skill.

Skipped when Rscript is missing or the upstream `survival` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("survival", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream survival package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("survival", {"fn": "does_not_exist_certainly_xyz"})
