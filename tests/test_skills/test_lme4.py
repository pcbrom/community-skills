"""Smoke tests for the lme4 skill.

Skipped when Rscript is missing or the upstream `lme4` package is
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream lme4 package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("lme4", {"fn": "does_not_exist_certainly_xyz"})
