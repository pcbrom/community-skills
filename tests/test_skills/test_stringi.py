"""Smoke tests for the stringi skill.

Skipped when Rscript is missing or the upstream `stringi` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("stringi", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream stringi package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("stringi", {"fn": "does_not_exist_certainly_xyz"})
