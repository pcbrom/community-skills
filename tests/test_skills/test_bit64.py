"""Smoke tests for the bit64 skill.

Skipped when Rscript is missing or the upstream `bit64` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("bit64", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream bit64 package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("bit64", {"fn": "does_not_exist_certainly_xyz"})
