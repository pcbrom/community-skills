"""Smoke tests for the tinytable skill.

Skipped when Rscript is missing or the upstream `tinytable` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("tinytable", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream tinytable package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("tinytable", {"fn": "does_not_exist_certainly_xyz"})
