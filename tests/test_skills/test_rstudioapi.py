"""Smoke tests for the rstudioapi skill.

Skipped when Rscript is missing or the upstream `rstudioapi` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("rstudioapi", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream rstudioapi package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("rstudioapi", {"fn": "does_not_exist_certainly_xyz"})
