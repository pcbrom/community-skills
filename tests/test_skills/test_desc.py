"""Smoke tests for the desc skill.

Skipped when Rscript is missing or the upstream `desc` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("desc", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream desc package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("desc", {"fn": "does_not_exist_certainly_xyz"})
