"""Smoke tests for the backports skill.

Skipped when Rscript is missing or the upstream `backports` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("backports", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream backports package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("backports", {"fn": "does_not_exist_certainly_xyz"})
