"""Smoke tests for the sass skill.

Skipped when Rscript is missing or the upstream `sass` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("sass", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream sass package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("sass", {"fn": "does_not_exist_certainly_xyz"})
