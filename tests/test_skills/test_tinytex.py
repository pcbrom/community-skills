"""Smoke tests for the tinytex skill.

Skipped when Rscript is missing or the upstream `tinytex` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("tinytex", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream tinytex package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("tinytex", {"fn": "does_not_exist_certainly_xyz"})
