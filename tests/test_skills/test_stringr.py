"""Smoke tests for the stringr skill.

Skipped when Rscript is missing or the upstream `stringr` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("stringr", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream stringr package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("stringr", {"fn": "does_not_exist_certainly_xyz"})
