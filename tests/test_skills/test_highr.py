"""Smoke tests for the highr skill.

Skipped when Rscript is missing or the upstream `highr` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("highr", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream highr package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("highr", {"fn": "does_not_exist_certainly_xyz"})
