"""Smoke tests for the roxygen2 skill.

Skipped when Rscript is missing or the upstream `roxygen2` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("roxygen2", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream roxygen2 package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("roxygen2", {"fn": "does_not_exist_certainly_xyz"})
