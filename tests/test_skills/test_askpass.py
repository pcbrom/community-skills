"""Smoke tests for the askpass skill.

Skipped when Rscript is missing or the upstream `askpass` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("askpass", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream askpass package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("askpass", {"fn": "does_not_exist_certainly_xyz"})
