"""Smoke tests for the gtable skill.

Skipped when Rscript is missing or the upstream `gtable` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("gtable", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream gtable package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("gtable", {"fn": "does_not_exist_certainly_xyz"})
