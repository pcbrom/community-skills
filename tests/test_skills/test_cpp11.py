"""Smoke tests for the cpp11 skill.

Skipped when Rscript is missing or the upstream `cpp11` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("cpp11", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream cpp11 package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("cpp11", {"fn": "does_not_exist_certainly_xyz"})
