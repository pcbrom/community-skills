"""Smoke tests for the processx skill.

Skipped when Rscript is missing or the upstream `processx` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("processx", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream processx package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("processx", {"fn": "does_not_exist_certainly_xyz"})
