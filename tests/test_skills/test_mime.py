"""Smoke tests for the mime skill.

Skipped when Rscript is missing or the upstream `mime` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("mime", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream mime package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("mime", {"fn": "does_not_exist_certainly_xyz"})
