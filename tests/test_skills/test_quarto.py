"""Smoke tests for the quarto skill.

Skipped when Rscript is missing or the upstream `quarto` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("quarto", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream quarto package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("quarto", {"fn": "does_not_exist_certainly_xyz"})
