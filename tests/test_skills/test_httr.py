"""Smoke tests for the httr skill.

Skipped when Rscript is missing or the upstream `httr` package is
...
def _pkg_installed() -> bool:
    ...
    completed = subprocess.run(
        ["Rscript", "-e", 'cat(requireNamespace("httr", quietly = TRUE))'],
...
pytestmark = pytest.mark.skipif(
    not _pkg_installed(),
    reason="R or upstream httr package not available",
)


def test_dispatcher_loads_and_rejects_unknown_fn():
    r = invoke("httr", {"fn": "does_not_exist_certainly_xyz"})
